...
export function registerCommands(
  ...
  recentDocsChangeEventEmitter: EventEmitter<void>
): void {
  ...
  commands.registerCommand('yuque.clearRecent', async () => {
    const flag = await window.showWarningMessage(`你确定清除最近浏览的数据吗？`, '确定', '取消')
    if (flag === '确定') {
      await settings.deleteRecentDocs()
      recentDocsChangeEventEmitter.fire()
    }
  ...
  commands.registerCommand(
    'yuque.openDoc',
    async (client: YuqueClient, namespace: string, doc: YuqueDoc) => {
      try {
        ...
        if (
          ...
          window.showWarningMessage(`抱歉，该文档的格式为".${docDetail.format}"，暂不支持查看。`)
        } else {
