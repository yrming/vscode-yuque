...
export class GroupsTreeProvider implements vscode.TreeDataProvider<GroupsTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<GroupsTreeItem | undefined | void> =
    new vscode.EventEmitter<GroupsTreeItem | undefined | void>()
  readonly onDidChangeTreeData: vscode.Event<GroupsTreeItem | undefined | void> =
  ...
  getTreeItem(
    element: GroupsTreeItem | ReposTreeItem
  ...
  async getChildren(
    element?: GroupsTreeItem | ReposTreeItem
  ): Promise<GroupsTreeItem[] | ReposTreeItem[] | DocsTreeItem[] | null | undefined> {
    if (element) {
  ...
  async getTopLevelTreeItems(): Promise<GroupsTreeItem[]> {
    ...
    if (Array.isArray(this.groups) && this.groups.length > 0) {
      const makeTreeItem = async (item: YuqueGroup) => {
        ...
        const treeItem = new GroupsTreeItem(
          item.name,
          item.id,
          item.description,
          fileUri,
