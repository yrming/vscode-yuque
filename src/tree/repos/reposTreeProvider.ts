...
export class ReposTreeProvider implements vscode.TreeDataProvider<ReposTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<ReposTreeItem | undefined | void> =
    new vscode.EventEmitter<ReposTreeItem | undefined | void>()
  readonly onDidChangeTreeData: vscode.Event<ReposTreeItem | undefined | void> =
  ...
  constructor(
      ...
      if (title) {
        ...
        if (treeItem.isPublic) {
          const value = await vscode.window.showQuickPick(
            [PublicTextEnum.Private, PublicTextEnum.Public],
            {
              placeHolder: '请选择可见范围'
            }
          )
    ...
    vscode.commands.registerCommand('yuque.repos.editDoc', async (treeItem: DocsTreeItem) => {
      try {
        let docDetail = await client.docs.get({
          namespace: treeItem.namespace,
          slug: treeItem.docId,
          data: { raw: 1 }
        })
        if (
          docDetail.format === 'lakeboard' ||
          docDetail.format === 'laketable' ||
          docDetail.format === 'lakeshow' ||
          docDetail.format === 'lakesheet' ||
          docDetail.format === 'lakemind'
        ) {
  ...
  async _addOrEditRepo(type: AddOrEditEnum, treeItem?: ReposTreeItem) {
    ...
    if (name) {
      const value = await vscode.window.showQuickPick(
        ['仅自己可见（自己和知识库成员可见）', '互联网可见（互联网所有人可见）'],
        {
          placeHolder: '请选择可见范围'
        }
      )
      if (value) {
        ...
        try {
          await this.client.repos.create({
            user: this.user.id,
            data: { name: name, slug: uuidv4(), type: 'Book', public: publicVal }
          })
  ...
  _createWebviewPanel(
    ...
    title: string,
    isPublic: number,
    repoNamesapce?: string,
    docStr?: string,
    storeToRecentObj?: RecentYuqueDoc
  ) {
            ...
            try {
              if (storeToRecentObj) {
                await this.client.docs.update({
                  namespace: repoNamesapce,
                  id: storeToRecentObj.id,
                  data: { title: title, public: isPublic, body: message.text }
                })
                ...
                await this.client.docs.create({
                  namespace: repoNamesapce,
                  data: { title: title, slug: uuidv4(), public: isPublic, body: message.text }
                })
              }
  ...
  async getChildren(
    element?: ReposTreeItem
  ): Promise<ReposTreeItem[] | DocsTreeItem[] | null | undefined> {
    if (element) {
  ...
  async getTopLevelTreeItems(): Promise<ReposTreeItem[]> {
    ...
    if (Array.isArray(this.repos)) {
      this.repos.forEach((item: YuqueRepo) => {
        const treeItem = new ReposTreeItem(
          item.name,
          item.namespace,
          item.public,
          item.name,
          vscode.TreeItemCollapsibleState.Collapsed,
          new vscode.ThemeIcon(item.public ? 'globe' : 'lock')
        )
