...
export class ProfileTreeProvider implements vscode.TreeDataProvider<ProfileTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<ProfileTreeItem | undefined | void> =
    new vscode.EventEmitter<ProfileTreeItem | undefined | void>()
  readonly onDidChangeTreeData: vscode.Event<ProfileTreeItem | undefined | void> =
  ...
  async getChildren(element?: ProfileTreeItem): Promise<ProfileTreeItem[] | null | undefined> {
    const treeItems: ProfileTreeItem[] = [
      ...
      new ProfileTreeItem(
        `账号类型：${this.user.type === 'Group' ? '团队' : '个人'}`,
        this.user.type === 'Group'
          ? new vscode.ThemeIcon('organization')
          : new vscode.ThemeIcon('person')
      ),
      new ProfileTreeItem(
        `关注者：${this.user.followers_count}`,
        new vscode.ThemeIcon('arrow-right')
      ),
      new ProfileTreeItem(
        `关注了：${this.user.following_count}`,
