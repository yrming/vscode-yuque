...
export async function registerTrees(
  context: vscode.ExtensionContext,
  recentDocsChangeEventEmitter: vscode.EventEmitter<void>
): Promise<void> {
  try {
    ...
    if (client && user) {
      const recentTreeProvider = new RecentTreeProvider(
        ...
        client,
        user,
        recentDocsChangeEventEmitter
      )
      vscode.window.registerTreeDataProvider('yuque.recent', recentTreeProvider)

      const reposTreeProvider = new ReposTreeProvider(
        context,
        client,
        user,
        recentDocsChangeEventEmitter
